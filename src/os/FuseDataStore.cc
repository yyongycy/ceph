...
int FuseDataStore::open_file(string p, struct fuse_file_info *fi,
...
FuseDataStore::FuseDataStore(ObjectStore *s, string p)
  : store(s),
    mount_point(p),
    fuse_thread(this)
{
                      ...
                      )
{
  ...
  FuseDataStore *fs = static_cast<FuseDataStore*>(fc->private_data);
...
static int os_open(const char *path, struct fuse_file_info *fi)
{
  ...
  if (pbl) {
    FuseDataStore::OpenFile *o = new FuseDataStore::OpenFile;
    o->bl = std::move(*pbl);
    fi->fh = reinterpret_cast<uint64_t>(o);
  }
  return 0;
...
static int os_release(const char *path, struct fuse_file_info *fi)
{
  ...
  ldout(fs->store->cct, 10) << __func__ << " " << path << dendl;
  std::lock_guard<std::mutex> l(fs->lock);
  FuseDataStore::OpenFile *o = reinterpret_cast<FuseDataStore::OpenFile*>(fi->fh);
  if (--o->ref == 0) {
                       ...
                       )
{
  ...
  if (fs->open_files.count(path)) {
    FuseDataStore::OpenFile *o = fs->open_files[path];
    if (o->bl.length() > size) {
      bufferlist t;
...
int FuseDataStore::start()
{
  ...
  fuse_thread.create("FuseDataStore");
  dout(10) << __func__ << " done" << dendl;
  return 0;
}

int FuseDataStore::loop()
{
